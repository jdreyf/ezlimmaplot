...
#' @param fdr.ext Extension for FDR columns; can be a regular expression.
#' @param sep Separator for column names before \code{p} or \code{FDR}.
#' @param pi0 Logical indicating if proportion of null hypotheses be calculated per p-value histogram. If \code{TRUE},
#' \code{limma::propTrueNull} is used, so \code{limma} package is needed.
...
sig_hist <- function(tab, p.ext='pval|p', fdr.ext='q|FDR', sep='.', pi0 = FALSE, name='sig_hist'){
  ...
  for (contr in prefix.v){
    ...
    if (!is.na(fdr.ext)){
      ...
      graphics::hist(tab[,fdr.col], xlab='FDR', main=contr)
    }
  }
