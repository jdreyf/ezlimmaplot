...
multi_heat <- function(tab, object, labrows=NULL, pheno.df=NULL, main="Log2 Expression", name="heats",
  ...
  for (contr in contr.names){
    ...
    ret.lst[[contr]] <- ezheat(object=object.tmp, labrows=labrows, pheno.df=pheno.df, main=main.tmp, name=NA, sc=sc, clip=clip,
                               ...
                               na.lab=na.lab, plot=plot, width=width, height=height)
  }
