...
test_that("non vdiffr",{
  ...
  expect_warning(mvl <- multi_volcano(tab=res.df, name=NA, ntop.sig = 1, ntop.lfc = 1, cut.lfc=1,
  ...
  expect_equal(ggplot2::ggplot_build(mvl$First3)$data[[1]]$colour[[1]], "green")
  expect_equal(ggplot2::ggplot_build(mvl$First3)$data[[1]]$y[1], -log10(mvl$First3$data["gene1", "First3.p"]))
  expect_equal(ggplot2::ggplot_build(mvl$Last3)$data[[1]]$colour[[1]], "black")
  expect_equal(ggplot2::ggplot_build(mvl$Last3)$data[[1]]$y[1], -log10(mvl$Last3$data["gene1", "Last3.p"]))
  expect_equal(ggplot2::ggplot_build(mvl$Last3vsFirst3)$data[[1]]$colour[[1]], "green")
  expect_equal(ggplot2::ggplot_build(mvl$Last3vsFirst3)$data[[1]]$y[1],
                     -log10(mvl$Last3vsFirst3$data["gene1", "Last3vsFirst3.p"]))

   mvl2 <-  multi_volcano(tab=res.df, name=NA, cut.lfc=1, cut.sig=0.01, cut.color = "green",
  ...
  expect_equal(ggplot2::ggplot_build( mvl2$First)$data[[1]]$colour, "green")
  expect_equal(ggplot2::ggplot_build( mvl2$First)$data[[1]]$y[1], -log10(mvl2$First3$data["gene1","First3.FDR"]))
  expect_equal(ggplot2::ggplot_build( mvl2$Last3)$data[[1]]$colour[1], "black")
  expect_equal(ggplot2::ggplot_build( mvl2$Last3)$data[[1]]$y[1], -log10(mvl2$Last3$data["gene1","Last3.FDR"]))
  expect_equal(ggplot2::ggplot_build( mvl2$Last3vsFirst3)$data[[1]]$colour, "green")
  expect_equal(ggplot2::ggplot_build(mvl2$Last3vsFirst3)$data[[1]]$y[1],
                       -log10(mvl2$Last3vsFirst3$data["gene1","Last3vsFirst3.FDR"]))

  expect_warning(mvlc3 <- multi_volcano(tab=res.df, name=NA, ntop.sig = 1, ntop.lfc = 1, cut.lfc=1,
  ...
  expect_equal(ggplot2::ggplot_build(mvlc3$First)$data[[1]]$y, -log10(mvlc3$First$data["gene1","First3.p"]))
  expect_equal(mvlc3$Last3$data["gene25","Last3.avg"], res.df["gene25","Last3.avg"])
  expect_equal(ggplot2::ggplot_build(mvlc3$Last3)$data[[1]]$y[1], -log10(mvlc3$Last3$data["gene1","Last3.p"]))
  expect_equal(mvlc3$Last3vsFirst3$data["gene25","Last3vsFirst3.avg"], res.df["gene25","Last3vsFirst3.avg"])
  expect_equal(ggplot2::ggplot_build(mvlc3$Last3vsFirst3)$data[[1]]$y[1],
  ...
  expect_equal(ggplot2::ggplot_build(mvlc3$First)$data[[1]]$colour, "black")
  expect_equal(ggplot2::ggplot_build(mvlc3$Last3)$data[[1]]$colour[1], "black")
  expect_equal(ggplot2::ggplot_build(mvlc3$Last3vsFirst3)$data[[1]]$colour, "black")
  ...
  expect_true(any(ggplot2::ggplot_build(mvlc4$First3)$data[[1]]$y == -log10(res.df["gene59", "First3.p"])))
   #Validating point C
  #Validating geom and stat arguments for the layers
  expect_equal(ggplot2::ggplot_build(mvlc4$Last3)$data[[2]]$label, res.df["gene74", "Gene.Symbol"])
  #Validating point D
  #Validating geom and stat arguments for the layers
  expect_equal(ggplot2::ggplot_build(mvlc4$Last3vsFirst3)$data[[1]]$y,
                      -log10(mvlc4$Last3vsFirst3$data["gene59", "Last3vsFirst3.p"]))
   #Validating the label
  expect_equal(mvlc4$First3$labels$label, "Gene.Symbol")
  expect_equal(mvlc4$Last3$labels$label, "Gene.Symbol")
  expect_equal(mvlc4$Last3vsFirst3$labels$label, "Gene.Symbol")
  #Validating the colour of the point
  expect_equal(ggplot2::ggplot_build(mvlc4$First3)$data[[1]]$colour, "black")
  expect_equal(ggplot2::ggplot_build(mvlc4$Last3)$data[[1]]$colour, "black")
  expect_equal(ggplot2::ggplot_build(mvlc4$Last3vsFirst3)$data[[1]]$colour, "red")
  ...
  #Validating the colour of the point
  expect_equal(ggplot2::ggplot_build(mvlc5$First3)$data[[1]]$colour, "green")
  expect_equal(ggplot2::ggplot_build(mvlc5$Last3)$data[[1]]$colour[1], "black")
  expect_equal(ggplot2::ggplot_build(mvlc5$Last3vsFirst3)$data[[1]]$colour, "green")

  mvlc6 <- multi_volcano(tab=res.df, name="Rplots", ntop.sig = 1, ntop.lfc = 1, cut.lfc=1, lab.col = "Gene.Symbol",
                      cut.sig=0.01, cut.color = "green", ann.rnames=c("gene1", "gene25"))
  expect_equal(ggplot2::ggplot_build(mvlc6$First3)$data[[2]]$label[1], res.df["gene1","Gene.Symbol"])
  expect_equal(ggplot2::ggplot_build(mvlc6$Last3)$data[[2]]$label[2], res.df["gene25","Gene.Symbol"])
  expect_equal(ggplot2::ggplot_build(mvlc6$Last3)$data[[4]]$label[1], res.df["gene74","Gene.Symbol"])
  expect_equal(ggplot2::ggplot_build(mvlc6$Last3vsFirst3)$data[[4]]$label[1], res.df["gene1","Gene.Symbol"])
  ...
  #Validating points
  expect_gte(ggplot2::ggplot_build(mvlc7$First3)$data[[1]]$y[1], 29)
  expect_equal(ggplot2::ggplot_build(mvlc7$Last3)$data[[2]]$label[2], res.df["gene25","Gene.Symbol"])
  expect_equal(ggplot2::ggplot_build(mvlc7$Last3)$data[[4]]$label[1], res.df["gene74","Gene.Symbol"])
  expect_equal(ggplot2::ggplot_build(mvlc7$Last3vsFirst3)$data[[4]]$label[1], res.df["gene1","Gene.Symbol"])

  #Validating the intercept point crossed by the straight line
  expect_equal(ggplot2::ggplot_build(mvlc7$First3)$data[[4]]$yintercept[1], -log10(0.05))
  expect_equal(ggplot2::ggplot_build(mvlc7$First3)$data[[1]]$y[1], -log10(mvlc7$First3$data["gene1", "First3.p"]))
  expect_equal(ggplot_build(mvlc7$First3)$data[[4]]$linetype, "dashed")

  expect_equal(ggplot2::ggplot_build(mvlc7$Last3)$data[[6]]$yintercept[1], -log10(0.05))
  expect_equal(ggplot2::ggplot_build(mvlc7$Last3)$data[[2]]$y[1], -log10(mvlc7$First3$data["gene1", "Last3.p"]))
  expect_equal(ggplot_build(mvlc7$Last3)$data[[6]]$linetype, "dashed")

  expect_equal(ggplot2::ggplot_build(mvlc7$Last3vsFirst3)$data[[6]]$yintercept[1], -log10(0.05))
  expect_equal(ggplot2::ggplot_build(mvlc7$Last3vsFirst3)$data[[4]]$y[1],
                        -log10(mvlc7$Last3vsFirst3$data["gene1", "Last3vsFirst3.p"]))
  expect_equal(ggplot_build(mvlc7$Last3vsFirst3)$data[[6]]$linetype, "dashed")
  ...
  expect_equal(ggplot_build(mvlc8$First3)$data[[4]]$linetype[1], "dashed")
  expect_equal(ggplot_build(mvlc8$First3)$data[[4]]$linetype[2], "dotted")
  expect_equal(ggplot2::ggplot_build(mvlc8$First3)$data[[4]]$yintercept[1], -log10(0.05))
  expect_equal(ggplot2::ggplot_build(mvlc8$First3)$data[[4]]$yintercept[2], -log10(0.001))

  expect_equal(ggplot_build(mvlc8$Last3)$data[[6]]$linetype[1], "dashed")
  expect_equal(ggplot_build(mvlc8$Last3)$data[[6]]$linetype[2], "dotted")
  expect_equal(ggplot2::ggplot_build(mvlc8$Last3)$data[[6]]$yintercept[1], -log10(0.05))
  expect_equal(ggplot2::ggplot_build(mvlc8$Last3)$data[[6]]$yintercept[2], -log10(0.001))

  expect_equal(ggplot_build(mvlc8$Last3vsFirst3)$data[[6]]$linetype[1], "dashed")
  expect_equal(ggplot_build(mvlc8$Last3vsFirst3)$data[[6]]$linetype[2], "dotted")
  expect_equal(ggplot2::ggplot_build(mvlc8$Last3vsFirst3)$data[[6]]$yintercept[1], -log10(0.05))
  expect_equal(ggplot2::ggplot_build(mvlc8$Last3vsFirst3)$data[[6]]$yintercept[2], -log10(0.001))

  mvlc9 <- multi_volcano(tab=res.df, ntop.sig = 1, ntop.lfc = 1, cut.lfc=1, name=NA, plot=FALSE,
                      cut.sig=0.01, cut.color = "green", ann.rnames=c("gene1", "gene25"), lines.sig = 0.05,
                      lab.col = "Gene.Symbol", up.ann.color = "blue")
  mvlc9.ann.mat.First3 <- ggplot2::ggplot_build(mvlc9$First3)$data[[2]]
  ...
  mvlc9.ann.mat.Last3 <- ggplot2::ggplot_build(mvlc9$Last3)$data[[2]]
  ...
  ev9.ann.mat.Last3vsFirst3 <- ggplot2::ggplot_build(mvlc9$Last3vsFirst3)$data[[2]]
  ...
  expect_equal(min(ggplot2::ggplot_build(mvlc12$First3)$data[[1]]$alpha), 1)
  expect_equal(max(ggplot2::ggplot_build(mvlc12$First3)$data[[1]]$alpha), 1)
  expect_equal(min(ggplot2::ggplot_build(mvlc12$Last3)$data[[1]]$alpha), 1)
  expect_equal(max(ggplot2::ggplot_build(mvlc12$Last3)$data[[1]]$alpha), 1)
  expect_equal(min(ggplot2::ggplot_build(mvlc12$Last3vsFirst3)$data[[1]]$alpha), 1)
  expect_equal(max(ggplot2::ggplot_build(mvlc12$Last3vsFirst3)$data[[1]]$alpha), 1)
