...
pheno <- data.frame(row.names = colnames(M), sample=colnames(M), grp, tissue, "covar num"=covar_num, covar2=covar_num,
...
We set `name=NA` to suppress plotting to a PNG file. We pass `color="grp"` via ... to **ggplot2** `aes_string`, where `"grp"` can be any of `colnames(pheno)}`. We could similarly have used `color="tissue"`. Passing `color="covar num"` is special, because **ggplot2** expects standard variable names or expressions of them, but we can do it with:
